...
function truncatedResultText(text: string, isError: boolean, headChars: number): string {
  ...
  const shortened = `${head}[fast-jev-compaction truncated ${text.length - headChars} chars of this tool result${
    isError ? ' (error)' : ''
  }; consult the original transcript if needed; do not repeat side-effecting actions]`;
