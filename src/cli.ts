...
and repeatable --requirement "fact to recover" (up to 6 complete, unique requirements; 1..240
characters each).
--requirement requires --jev because requirements affect Jev reranking only.
...
async function main() {
  ...
  if (!['inspect', 'archive', 'search', 'retrieve', 'status', 'compact'].includes(command)) throw new Error('Unknown command; use --help');
  ...
  if (command === 'search' || command === 'retrieve' || command === 'status') {
    ...
    if (command === 'search') {
      ...
      if (values.query.length > 4_000) throw new Error('Query exceeds 4000 characters');
      if ((values['task-context']?.length ?? 0) > 2000) throw new Error('Task context exceeds 2000 characters');
      const limit = integer(values.limit, 10, 1, 100, '--limit');
      if (values.requirement?.length && !values.jev) throw new Error('--requirement requires --jev');
      ...
      const result = values.jev
        ? await rankArchiveSearch(local, values.query, new JevClient(), limit, {
          taskContext: values['task-context'], requirements: values.requirement,
        })
      ...
      if (offset > serialized.length) throw new Error('Offset exceeds evidence length');
  ...
  if (command === 'compact') {
    try { await stat(values.output!); throw new Error('Output already exists; choose a new path'); }
  ...
  if (command === 'inspect') {
    let isCatalog = false;
    try { isCatalog = JSON.parse(text)?.version === 2; } catch { /* JSONL */ }
    if (isCatalog) {
      const catalog = await readCatalog(values.input);
      console.log(JSON.stringify({ version: 2, entries: catalog.entries.length,
        currentEntries: catalog.currentIds.length, byKind: catalog.entries.reduce<Record<string, number>>((a, e) => {
