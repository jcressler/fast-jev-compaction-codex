...
import { buildEvidenceEntries, type EvidenceSummary } from './evidence.js';

export const MAX_ARCHIVE_BYTES = 64 * 1024 * 1024;
export interface ArchiveIdentity { session: string; transcript: string; cwd: string }
...
export async function readBounded(path: string): Promise<string> {
  ...
  if (!info.isFile() || info.size > MAX_ARCHIVE_BYTES) throw new Error('Unsupported archive or transcript size');
  const content = await readFile(path, 'utf8');
  if (Buffer.byteLength(content) > MAX_ARCHIVE_BYTES) throw new Error('Archive or transcript grew beyond limit');
  return content;
}

export async function atomicJson(path: string, value: unknown): Promise<void> {
  const content = JSON.stringify(value);
  if (Buffer.byteLength(content) > MAX_ARCHIVE_BYTES) throw new Error('Archive index exceeds 64 MiB');
...
async function immutableFile(path: string, content: string): Promise<void> {
  const temporary = `${path}.${randomUUID()}.tmp`;
  try {
    await writeFile(temporary, content, { flag: 'wx', mode: 0o600 });
    try { await link(temporary, path); }
      ...
      if (await readBounded(path) !== content) throw new Error('Immutable archive object or identity conflict');
    }
  } finally { await rm(temporary, { force: true }); }
}

function validateCatalog(value: EvidenceCatalog): EvidenceCatalog {
  if (!value || value.version !== 2 || !value.identity ||
      !['session', 'transcript', 'cwd'].every(key => typeof value.identity[key as keyof ArchiveIdentity] === 'string') ||
      ...
      !value.currentIds.every(validId)) throw new Error('Invalid evidence catalog');
  const ids = new Set(value.entries.map(e => e.id));
  if (ids.size !== value.entries.length || value.currentIds.some(id => !ids.has(id))) throw new Error('Invalid catalog references');
  return value;
}

function decodeCatalog(text: string): EvidenceCatalog {
  try { return validateCatalog(JSON.parse(text)); }
  catch { throw new Error('Invalid evidence catalog'); }
}

function sameIdentity(a: ArchiveIdentity, b: ArchiveIdentity): boolean {
...
async function mergeCaptures(directory: string, seed: EvidenceCatalog): Promise<EvidenceCatalog> {
  const captures = [seed];
  let names: string[];
  try { names = await readdir(join(directory, 'captures')); }
  ...
  for (const name of names) {
    ...
    if (!sameIdentity(capture.identity, seed.identity)) throw new Error('Archive identity mismatch');
    captures.push(capture);
  }
  captures.sort((a, b) => a.updated - b.updated || a.generation.localeCompare(b.generation));
  const entries = new Map<string, EvidenceSummary>();
  for (const capture of captures) for (const entry of capture.entries) entries.set(entry.id, entry);
  return { ...captures[captures.length - 1]!, created: Math.min(...captures.map(c => c.created)), entries: [...entries.values()] };
}

export async function readCatalog(path: string): Promise<EvidenceCatalog> {
...
export async function captureArchive(
  items: CodexItem[], directory: string, identity: ArchiveIdentity, now = Date.now(),
  ...
  let previous: EvidenceCatalog | undefined;
  try { previous = await readCatalog(path); }
  ...
  for (const entry of incoming) {
    ...
    if (id !== entry.id || Buffer.byteLength(content) > MAX_ARCHIVE_BYTES) throw new Error('Invalid evidence object');
...
export async function retrieveEvidence(catalogPath: string, id: string) {
  if (!validId(id)) throw new Error('Evidence ID must be a full SHA-256 identifier');
  const catalog = await readCatalog(catalogPath);
  const entry = catalog.entries.find(e => e.id === id);
  if (!entry) throw new Error('Evidence ID not found in this archive');
  const content = await readBounded(join(dirname(resolve(catalogPath)), 'objects', `${id}.json`));
  if (createHash('sha256').update(content).digest('hex') !== id) throw new Error('Evidence object integrity check failed');
  ...
  return { entry, records: records as CodexItem[] };
}
