...
describe('state fitting', () => {
  ...
  it('truncates tool inputs before touching message text', () => {
    ...
    expect(['inputs<=200', 'inputs<=60']).toContain(stage);
...
describe('decisions', () => {
  ...
  it('honours truncateHeadChars, including a zero head', () => {
    ...
    expect(kept[2]?.toolResults?.[0]?.text).toBe(
      `${original.slice(0, 50)}\n[fast-jev-compaction truncated ${total - 50} chars of this tool result; consult the original transcript if needed; do not repeat side-effecting actions]`,
    );
    ...
    expect(noHead[2]?.toolResults?.[0]?.text).toBe(
      `[fast-jev-compaction truncated ${total} chars of this tool result; consult the original transcript if needed; do not repeat side-effecting actions]`,
    );
  });
});
