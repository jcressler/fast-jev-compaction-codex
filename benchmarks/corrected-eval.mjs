#!/usr/bin/env node
/** Corrected Jev trial: explicit requirements, binary questions, coverage selection. */
...
export const PROTOCOL = Object.freeze({ schemaVersion: 3, experiment: 'corrected-jev', model: 'gpt-5.6-luna', effort: 'medium',
  ...
  stoppingRule: { quality: 'At least 2 more fully correct runs than BOTH comparators, wins in at least 2 distinct cases versus each, and no case lost in both repetitions.',
    ...
    guard: 'No additional hard-constraint regression test failures versus either comparator. All 24 runs and all 4 Jev requests must complete. Otherwise this experiment does not demonstrate added value; it does not establish that Jev is ineffective.' },
  changes: ['Binary Noul propositions with explicit true/false criteria.',
    'Every arm and local candidate search receives the complete requested factual requirements; Jev receives these same requirements without expected answers.',
    'Jev asks atomic candidate/requirement questions and code orders evidence by marginal requirement coverage.',
    'Four fresh cases with historical configuration needed for code and numeric/boolean/identifier facts, avoiding paraphrase grading.',
    'Synthetic Jev request state/questions and returned probability values are recorded for audit.'],
  limitations: ['Generated small coding-maintenance tasks, not real repository work or production acceptance.',
    ...
    'This compares a combined corrected integration; it does not isolate the effects of prompt wording, requirements, and coverage ordering individually.',
    ...
    'Success thresholds are practical signals, not statistical significance or a universal claim about Jev.'] });
...
/** User-visible field descriptions, never evaluator answers, reach all arms. */
export function taskInputs(testCase) {
  const requirements = testCase.requirements;
  if (!Array.isArray(requirements) || requirements.length !== 3 || requirements.some(value => typeof value !== 'string' || !value.trim())) throw new Error('Missing explicit factual requirements');
  const requested = `Required historical facts:\n${requirements.map(value => `- ${value}`).join('\n')}`;
  const taskContext = `${testCase.taskContext}\n\n${requested}`;
  const query = `${testCase.query}\n\n${requested}`;
  if (taskContext.length > 2000 || query.length > 1000) throw new Error('Visible task exceeds reranker context bounds');
  return { taskContext, query, requirements };
}
...
export function scoreCode(source, tests) {
  ...
  const response = spawnSync(process.execPath, ['--max-old-space-size=64', fileURLToPath(new URL('./corrected-code-worker.mjs', import.meta.url))],
...
export async function prepareCase(testCase, root, asker) {
  ...
  const inputs = taskInputs(testCase);
  const items = [{ type: 'message', role: 'user', content: inputs.taskContext }, ...testCase.stages.flat()];
  ...
  const local = await searchArchive(catalogPath, inputs.query, { limit: PROTOCOL.candidateLimit });
  ...
  if (asker) {
    const wrapped = { ask: async (state, questions) => {
      ...
      telemetry.request = { state, questions };
      ...
      telemetry.answers = Object.fromEntries(Object.keys(questions).map(key => [key, { noul: response.answers?.[key]?.noul ?? null }]));
      return response;
    } };
    const ranked = await rankArchiveSearch(local, inputs.query, wrapped, PROTOCOL.candidateLimit, { taskContext: inputs.taskContext, requirements: inputs.requirements });
...
async function continueCase(testCase, prepared, binary, caseIndex, save) {
  ...
  try {
    ...
    await client.request('thread/inject_items', { threadId: baseId, items: wire([{ type: 'message', role: 'user', content: inputs.taskContext }]) });
...
export function decision(reports) {
  ...
  return { outcome: successfulJev && (Object.values(comparisons).every(row => row.quality) || Object.values(comparisons).every(row => row.efficiency)) ? 'added-value-signal' : 'no-demonstrated-benefit', comparisons };
}

export async function runCorrected({ live = false, allowNetwork = false, binary, runDir } = {}) {
  ...
  const root = runDir ? resolve(runDir) : await mkdtemp(join(tmpdir(), 'fast-jev-corrected-'));
  ...
  const protocol = { ...PROTOCOL, live, fixtureHashes: cases.map(item => ({ id: item.id, stages: digest(item.stages),
    ...
    expectedSource: digest(item.expectedSource), expectedFacts: digest(item.expectedFacts), schema: digest(item.answerSchema), requirements: digest(item.requirements) })) };
  protocol.implementationHashes = Object.fromEntries(await Promise.all(['corrected-eval.mjs', 'corrected-fixtures.mjs', 'corrected-code-worker.mjs',
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  ...
  runCorrected({ live: args.includes('--live'), allowNetwork: args.includes('--allow-network'), binary: args.includes('--codex') ? value('--codex') : undefined,
    ...
    .catch(() => { console.error('Corrected evaluation failed; inspect the saved aggregate report. Credentials and native payloads are not logged.'); process.exitCode = 1; });
}
