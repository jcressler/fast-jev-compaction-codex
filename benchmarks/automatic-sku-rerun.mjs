...
const runDir = resolve(value('--run-dir', 'benchmarks/automatic-eval-run'));
const outputPath = value('--output', join(runDir, 'report.json'));
const codex = value('--codex', process.env.CODEX_BINARY ?? 'codex');
const fixturePath = resolve(fileURLToPath(new URL('./automatic-fixtures-v2.mjs', import.meta.url)));
...
const amendmentPath = join(repoRoot, 'benchmarks', 'AUTOMATIC-AMENDMENT-2026-09-18.md');
...
if (manifest.amendmentHash !== frozen.amendmentHash) throw new Error('Frozen amendment changed');
...
for (const taskIndex of taskIndices) {
  for (let repetition = 0; repetition < repetitionCount; repetition += 1) {
    const task = await fixture.createTask(taskIndex, repetition + 1);
    for (const mode of manifest.armOrder[taskIndices.indexOf(taskIndex) * repetitionCount + repetition]) {
      try {
      ...
      console.log(JSON.stringify({ event: 'arm-completed', completed: reports.length, total: taskCount * repetitionCount * armNames.length, taskIndex, repetition, mode, status: reports.at(-1).status, integrationValid: reports.at(-1).integrationValid }));
    }
  }
}
